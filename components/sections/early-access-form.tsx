...
export function EarlyAccessForm() {
  ...
  const {
    ...
    defaultValues: { country: 'US', assetType: [''] },
  })
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      let res = await fetch('/api/form/early-access-more-info', {
        method: 'POST',
  ...
  return (
    <Container>
      {!isEmpty(errors) && (
        <div className="mx-auto mt-8 flex max-w-2xl flex-col items-start space-y-2 rounded-md bg-red-100 p-4">
      ...
      <form
        ...
        action=""
        className="mx-auto mb-10 flex max-w-2xl flex-col pt-10"
      >
        <input
          id="honey"
          type="hidden"
        ...
        <label className="mb-5 flex flex-col items-start">
        ...
        <label className="mb-5 flex flex-col items-start">
          <span className="text-gray-700">Your country</span>
          <select
            className="block w-full appearance-none rounded-sm border border-gray-300 px-3 py-2 placeholder-gray-400 transition duration-200 ease-in-out focus:border-black focus:outline-none focus:ring-black sm:text-sm"
        ...
        <label className="mb-5 flex flex-col items-start">
          <span className="text-gray-700">
            What tools or software do you currently use to analyze deals or
            underwrite property?
          </span>
          <textarea
            className="block w-full appearance-none rounded-sm border border-gray-300 px-3 py-2 placeholder-gray-400 transition duration-200 ease-in-out focus:border-black focus:outline-none focus:ring-black sm:text-sm"
            rows={8}
            placeholder="Excel, Google Sheets, etc."
            {...register('tools', { required: false })}
          />
        </label>
        <label className="mb-5 flex flex-col items-start">
          <span className="pb-4 text-gray-700">
            What types of real estate assets do you invest in?
          </span>

          <label className="mb-5 flex items-center space-x-2">
            <input
              className="bg-full-black text-teal border-gray focus:border-teal focus:ring-teal h-6 w-6 rounded border-2 focus:ring-2"
              type="checkbox"
              placeholder="Single Family Residential"
              value="Single Family Residential"
              {...register('assetType', { required: false })}
            />
            <span className="text-gray-700">Single Family Residential</span>
          </label>
          <label className="mb-5 flex items-center space-x-2">
            <input
              className="bg-full-black text-teal border-gray focus:border-teal focus:ring-teal h-6 w-6 rounded border-2 focus:ring-2"
              type="checkbox"
              placeholder="Multifamily Residential"
              value="Multifamily Residential"
              {...register('assetType', { required: false })}
            />
            <span className="text-gray-700">Multifamily Residential</span>
          </label>
          <label className="mb-5 flex items-center space-x-2">
            <input
              className="bg-full-black text-teal border-gray focus:border-teal focus:ring-teal h-6 w-6 rounded border-2 focus:ring-2"
        ...
        <label className="mb-5 flex flex-col items-start">
          <span className="text-gray-700">
            Any other comments, suggestions, thoughts, etc?
          </span>
          <textarea
            className="block w-full appearance-none rounded-sm border border-gray-300 px-3 py-2 placeholder-gray-400 transition duration-200 ease-in-out focus:border-black focus:outline-none focus:ring-black sm:text-sm"
            rows={8}
        ...
        >
          ...
          {router.query.email ? 'Update my early access info' : 'Submit'}
        </button>
      </form>
    </Container>
  )
